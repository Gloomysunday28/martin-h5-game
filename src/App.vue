<template>
  <div id="app" ref="contain">
    <span :class="['header-back', {'header-back--board': YGBOARD}]" @click="back" v-if="!hasNoBack">
...
export default {
  ...
  mounted() {
    const touch = window.touchStart ? 'touchStart' : 'click'
    document.addEventListener(touch, this.beforeStart, false)
  ...
  methods: {
    ...
    love() {
      ...
      love.className = 'iconfont'
      love.innerHTML = '&#xe605;'
      love.style.color = `rgba(${~~(Math.random() * 255)}, ${~~(Math.random() * 255)}, ${~~(Math.random() * 255)}, 1)`
      return love
    },
    // 添加小爱心到所点的位置
    beforeStart(e) {
      const love = this.love()
      const showX = e.clientX || e.touches[0].offsetX
      const showY = e.clientY || e.touches[0].offsetY
      love.style.left = showX + 'px'
      love.style.top = showY + 'px'
      love.classList.add('icon-love')
      this.$refs.contain.appendChild(love)
